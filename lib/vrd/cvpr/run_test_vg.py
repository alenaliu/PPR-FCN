...
def run_test_visualize():
    ...
    net = caffe.Net('models/vg1_2/vgg16/faster_rcnn_end2end/test.prototxt',
                    'output/faster_rcnn_end2end/vg1_22016_train/vg1_2_vgg16_faster_rcnn_finetune_iter_150000.caffemodel',caffe.TEST)
    # net.name = os.path.splitext(os.path.basename(args.caffemodel))[0]
    # net.name = 'vg1_2'
    # imdb = get_imdb('vg1_2_2016_test')
    # imdb.competition_mode(0)
    # if not cfg.TEST.HAS_RPN:
        # imdb.set_proposal_method(cfg.TEST.PROPOSAL_METHOD)
...
def run_test_object_detection_eval():
    caffe.set_mode_gpu()
    caffe.set_device(0)
    h5f = h5py.File('output/vg_object_detections_55k.hdf5')
    m = h5py.File('/home/zawlin/Dropbox/proj/vg1_2_meta.h5', 'r', 'core')

    net = caffe.Net('models/vg1_2/vgg16/faster_rcnn_end2end/test.prototxt',
                    'output/models/vg1_2_vgg16_faster_rcnn_finetune_iter_55000.caffemodel',caffe.TEST)
    # net = caffe.Net('models/vg1_2/vgg16/faster_rcnn_end2end/test.prototxt',
...
def gen_obj_detection_results_from_hdf5(h5_path,out_path):
    ...
    for k in h5f.keys():
        ...
        for j in xrange(1, 101):
            ...
            cls_dets = np.hstack((cls_boxes, cls_scores[:, np.newaxis])) \
                .astype(np.float32, copy=False)
            keep = nms(cls_dets, .8, force_cpu=True)  # nms threshold
            cls_dets = cls_dets[keep, :]
            boxes_tosort.append(cls_dets)
        for j in xrange(len(boxes_tosort)):
            cls_dets = boxes_tosort[j]
...
#gen_obj_detection_results_from_hdf5('output/vr_object_detections.hdf5','output/vr_object_detections.txt')
