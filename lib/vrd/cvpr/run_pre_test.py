...
def run_relation(model_type,iteration):
    vgg_data = h5py.File('output/sg_vrd_2016_test_predicate_exp_test.hdf5')
    ...
    for imid in vgg_data.keys():
        ...
        sub_visual = vgg_data[imid]['sub_classeme']
        obj_visual = vgg_data[imid]['obj_classeme']
        ...
        for s in xrange(sub_boxes_gt.shape[0]):
            if model_type=='pre_diff':
                ...
                pass
            elif model_type=='pre_concat_good':
                visual = np.hstack((sub_visual[s], obj_visual[s])).reshape(1,202)
                blob = {
                    'visual':visual
                    }
            #print visual.shape
