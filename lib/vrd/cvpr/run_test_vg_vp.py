...
def convert_vp_result_for_matlab_eval2():
    ...
    for imid in imlist.keys():
        ...
        # print 'im_detect: {:d} {:.3f}s {:.3f}s' \
            # .format(cnt, _t['im_detect'].average_time,
                    # _t['misc'].average_time)
def convert_vp_result_for_matlab_eval():
    _t = {'im_detect': Timer(), 'misc': Timer()}
    m_vp = h5py.File('/home/zawlin/Dropbox/proj/vg1_2_vp_meta.h5', 'r', 'core')
    m = h5py.File('/home/zawlin/Dropbox/proj/vg1_2_meta.h5', 'r', 'core')
    root = 'data/vg1_2_2016/Data/test/'
    ...
    for imid in imlist.keys():
        ...
        for j in xrange(1, 19237):
            ...
            if cls_scores.shape[0]==0:continue
            cls_dets = np.hstack((cls_boxes, cls_scores[:,np.newaxis]))
            keep = nms(cls_dets, .2,force_cpu=True)  # nms threshold
            cls_dets = cls_dets[keep, :]
            boxes_tosort.append(cls_dets)
            h5_boxes.append(cls_dets[:,:4])
            h5_confs.append(cls_dets[:,4])
            h5_labels.append(np.tile(j,cls_dets.shape[0]))
            # if h5_boxes == None:
                # h5_boxes = cls_dets[:,:4]
                # h5_confs = cls_dets[:,4]
                # h5_labels = np.tile(j,cls_dets.shape[0])
            # else:
                # h5_boxes = np.vstack((h5_boxes,cls_dets[:,:4]))
                # h5_confs = np.hstack((h5_confs,cls_dets[:,4]))
                # h5_labels = np.hstack((h5_labels, np.tile(j,cls_dets.shape[0])))
        #print h5_boxes
        h5_boxes = np.concatenate(h5_boxes)
        h5_confs = np.concatenate(h5_confs)
        h5_labels = np.concatenate(h5_labels)
        h5f_nms.create_dataset(imid_orig+'/boxes',data = h5_boxes)
        h5f_nms.create_dataset(imid_orig+'/confs',data = h5_confs)
        h5f_nms.create_dataset(imid_orig+'/labels',data = h5_labels)
...
def make_matlab_from_vp_nms2():
    ...
    # boxes_ours.append([])
    # boxes_ours=boxes_ours[:-1]
    sio.savemat('output/vg1_2_vp_results_.4.mat', {'bboxes_ours': boxes_ours,
        'rlp_labels_ours':rlp_labels_ours,'rlp_confs_ours':rlp_confs_ours})#'relation_vectors':relation_vectors})
