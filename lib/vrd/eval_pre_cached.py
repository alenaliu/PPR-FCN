...
def im_detect(net, im_path, conv_new_1,sub_boxes_gt,obj_boxes_gt):
    ...
    blobs['conv_new_1'] = conv_new_1
    blobs['sub_boxes'] = (sub_boxes*im_scales[0]).reshape(-1,5,1,1)
    blobs['obj_boxes'] = (obj_boxes*im_scales[0]).reshape(-1,5,1,1)

    # reshape network inputs
    net.blobs['conv_new_1'].reshape(*(blobs['conv_new_1'].shape))
    net.blobs['sub_boxes'].reshape(*(blobs['sub_boxes'].shape))
    net.blobs['obj_boxes'].reshape(*(blobs['obj_boxes'].shape))

    forward_kwargs = {'conv_new_1': blobs['conv_new_1'],
...
def run_relation(model_type,iteration):
    ...
    keep = 100
    thresh = 0.0001
    prototxt = 'models/sg_vrd/rel_iccv/test_'+model_type+'.prototxt'
    model_path ='output/sg_vrd_rfcn/'+model_type+'_iter_'+iteration+'.caffemodel'
    ...
    for imid in cache_h5.keys():
        ...
        for s in xrange(sub_boxes_gt.shape[0]):
            ...
            im_detect(net,im_path,conv_new_1,obj_boxes_gt,sub_boxes_gt)
