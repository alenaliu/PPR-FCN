...
def run_relation(model_type,iteration):
    ...
    for imid in cache_h5.keys():
        ...
        if boxes.shape[0]>=2:
            for s in xrange(boxes[:200].shape[0]):
                for o in xrange(boxes[:200].shape[0]):
                    if s == o:continue
                    if boxes[s][4]<0.001:continue
                    if boxes[o][4]<0.001:continue
                    sbox = boxes[s][:4]
                    obox = boxes[o][:4]
            ...
            im_detect(net,im_path,conv_new_1,sub_boxes,obj_boxes)
...
# model_type = 'pre_psroi_context_tri_sum_cached'
# iteration = '15000'
