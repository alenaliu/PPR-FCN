...
def load_ebcache():
    ...
    for i in h5f['train']:
        cnt += 1
        if cnt %1000 ==0:
            glog.info('loading eb cache %d'%cnt)
        data=h5f['train/%s'%i][...].astype(np.float32)
        idx = np.argsort(data[:,-1],axis=0)
        data_sorted = data[idx][::-1]
        data_sorted_idx = np.where((data_sorted[:,2]-data_sorted[:,0]>25) & (data_sorted[:,3]-data_sorted[:,1]>25))
        data_sorted = data_sorted[data_sorted_idx]
        eb_cache[i] = data_sorted[...]
        #print data_sorted
        #h5_rois[i] = data_sorted[:60,:4]
    glog.info('eb cache loaded.')

def load_ebcache_rerank():
    global eb_cache
    eb_cache = {}
    h5path =  'output/precalc/sg_vrd_obj_rerank_wsd.h5'
    h5f = h5py.File(h5path,driver='core')
    cnt = 0
    for i in h5f['train']:
        cnt += 1
        if cnt %1000 ==0:
            glog.info('loading eb cache %d'%cnt)
        data=h5f['train/%s/boxes'%i][...].astype(np.float32)
        idx = np.argsort(data[:,-1],axis=0)
        data_sorted = data[idx][::-1]
        data_sorted_idx = np.where((data_sorted[:,2]-data_sorted[:,0]>20) & (data_sorted[:,3]-data_sorted[:,1]>20))
        data_sorted = data_sorted[data_sorted_idx]
        eb_cache[str(i)] = data_sorted[...]
        #print data_sorted
        #h5_rois[i] = data_sorted[:60,:4]
    glog.info('eb cache loaded.')

def load_ebcache_rerank_db():
    global eb_cache
    eb_cache = {}
    h5path =  'output/precalc/sg_vrd_obj_rerank_wsd_db.h5'
    h5f = h5py.File(h5path,driver='core')
    cnt = 0
    for i in h5f['train']:
        cnt += 1
    ...
    if cfg.TRAIN.HAS_RPN:
        ...
        if cfg.TRAIN.USE_EB:
            ...
            if eb_cache ==None:
                #load_ebcache()
                #load_ebcache_rerank()
                load_ebcache_rerank_db()
            labels = gt_boxes[:,4]
            blobs['labels']=labels
            imid = roidb[0]['image'].split('/')[-1][:-4]
            eb_boxes = eb_cache[imid]
            np.random.shuffle(eb_boxes)

            blobs['gt_boxes'] = eb_boxes[:1000,0:4]*im_scales[0]
        else:
