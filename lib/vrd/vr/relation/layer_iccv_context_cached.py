...
class ICCVDataLayerContextCached(caffe.Layer):
    ...
    def get_minibatch(self):
        ...
        gt_sub_boxes = gt['sub_boxes']
        gt_obj_boxes = gt['obj_boxes']
        gt_rlp_labels = gt['rlp_labels']
        boxes = self.cache_boxes[imid]
        sub_boxes = []
        obj_boxes = []
        labels = []
        for i in xrange(gt_sub_boxes.shape[0]):
            sub_box = gt_sub_boxes[i]
            obj_box = gt_obj_boxes[i]
            label = gt['rlp_labels'][i,1]

            overlaps = bbox_overlaps(
                np.ascontiguousarray([sub_box, obj_box], dtype=np.float),
                np.ascontiguousarray(boxes[:,:4], dtype=np.float))


            sub_sorted = overlaps[0].argsort()[-30:][::-1]
            obj_sorted = overlaps[1].argsort()[-30:][::-1]
            while len(sub_sorted) > 0 and overlaps[0][sub_sorted[-1]] < .5: sub_sorted = sub_sorted[:-1]
            while len(obj_sorted) > 0 and overlaps[1][obj_sorted[-1]] < .5: obj_sorted = obj_sorted[:-1]

            for s in sub_sorted[:1]:  # sub_idx:
                for o in obj_sorted[:1]:  # obj_idx:
                    if s!=0:
                        sub_boxes.append(boxes[s,:4])
                        obj_boxes.append(boxes[o,:4])
                        labels.append(label)
        ...
        if imid not in self.cache_mem:
            self.cache_mem[imid] = self.cache['train/%s/conv_new_1'%imid][...]
        blobs['conv_new_1']=self.cache_mem[imid]
        sub_boxes = sub_boxes*im_scales[0]
        obj_boxes = sub_boxes*im_scales[0]
        ...
        blobs['labels'] = labels#gt['rlp_labels'][:,1]
