...
class vg1_2_vp(imdb):
    def __init__(self, image_set, year):
        imdb.__init__(self, 'vg1_2_vp' + year + '_' + image_set)
        ...
        if 'test' in self._image_set:self._folder_path = 'test'
        if 'train' in self._image_set:self._folder_path = 'train'
        if 'mini' in self._image_set:self._folder_path = 'train'
        ...
        self.m = h5py.File('/home/zawlin/Dropbox/proj/vg1_2_vp_meta.h5','r','core')
        for i in range(len(self.m['meta/tri/idx2name/'])):
            self._classes += (str(self.m['meta/tri/idx2name/'+str(i)][...]),)
            self._class_name += (str(self.m['meta/tri/idx2name/'+str(i)][...]),)
            self._class_ids += (str(self.m['meta/tri/idx2name/'+str(i)][...]),)
        ...
        assert os.path.exists(self._data_path), \
            'vg1_2 data path does not exist: {}'.format(self._data_path)
        if self._image_set != 'test':
            assert os.path.exists(self._annot_path), \
                'vg1_2 annotation path does not exist: {}'.format(self._annot_path)

    def _get_default_path(self):
        """
        Return the default path where IMAGENET is expected to be installed.
        """
        return os.path.join(cfg.DATA_DIR, 'vg1_2_vp_' + self._year)
