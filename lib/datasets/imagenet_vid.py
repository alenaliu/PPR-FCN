...
class imagenet_vid(imdb):
    def __init__(self, image_set, year):
        ...
        if 'val' in self._image_set:self._folder_path = 'val'

        self._devkit_path = os.path.join(self._get_default_path(), 'devkit')
        self._data_path = os.path.join(self._get_default_path(), 'Data', 'VID', self._folder_path)
        self._annot_path = os.path.join(self._get_default_path(), 'Annotations', 'VID', self._folder_path)
        ...
        synsets = sio.loadmat(os.path.join(self._devkit_path, 'data', 'meta_vid.mat'))
    ...
    def _get_default_path(self):
        ...
        """
        return os.path.join(cfg.DATA_DIR, 'ilsvrc' + self._year)
    ...
    def _load_image_set_index(self):
        ...
        if self._image_set=='train':
            image_set_file = os.path.join(self._get_default_path(), 'ImageSets', 'VID',
    ...
    def _load_imagenet_annotation(self, index):
        ...
        objs_filtered=[]
        for o in objs:
            if str(get_data_from_tag(o, "name")).lower().strip() in self._class_to_ind:
